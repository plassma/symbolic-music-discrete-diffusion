...
BUILD_DIR = "./extracts" # Output path
SOUNDFONT = "SGM-v2.01-Steinway-Guit-Bass-v2.9.sf2"#"Essential Keys-sforzando-v9.6.sf2"# Soundfont file path
PYTHON_PATH = "/home/plassma/miniconda3/envs/symbolic-music-discrete-diffusion/bin/python"# python with note_seq installed
...
def midi_to_audio(source, target)
  ...
  system("#{PYTHON_PATH} drumify.py #{source}")

  #run_command "python drumify.py #{source}"
  run_command "#{FLUIDSYNTH} -C no -R no -g 0.5 -F #{target} #{SOUNDFONT} #{source}"
  run_command "#{OGGENC} -b 128 #{target}"
  run_command "#{LAME} -v -b 128 #{target}"
  rm target
end
